...
#pragma mark - 0.5秒后显示失败HUD
+ (void)afterShowErrorWithStatus:(NSString *)status
{
    [SVProgressHUDManager dismiss];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [SVProgressHUDManager showErrorWithStatus:status];
    });
}
...
#pragma mark - 0.5秒后显示成功HUD
+ (void)afterShowSuccessWithStatus:(NSString *)status
{
    [SVProgressHUDManager dismiss];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [SVProgressHUDManager showSuccessWithStatus:status];
    });
}
...
#pragma mark - 0.5秒后显示HUD
+ (void)afterInfoWithStatus:(NSString *)status
{
    [SVProgressHUDManager dismiss];
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [SVProgressHUDManager showInfoWithStatus:status];
    });
}
